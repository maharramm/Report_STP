...
namespace ReportSTP.Controllers.Report
{
    public class CurrentAccountBalanceController : Controller
    {
        public IActionResult Index()
        {
            ...
            string query = @"
SELECT     CLC.DEFINITION_,CLC.CODE,A.Qaliq,A.VALYUTA,A.ELEMAN   FROM (
...
CASE WHEN CLCARD.CCURRENCY=0 THEN 'AZN' 
...
";
            ...
            {
                reportList.Add(new CurrentAccountBalance
                {
                    Definition = dataTable.Rows[i]["DEFINITION_"].ToString(),
                    Code = dataTable.Rows[i]["CODE"].ToString(),
                    Remainder = !string.IsNullOrEmpty(dataTable.Rows[i]["Qaliq"].ToString()) ? Convert.ToDouble(dataTable.Rows[i]["Qaliq"].ToString()) : 0.0,
